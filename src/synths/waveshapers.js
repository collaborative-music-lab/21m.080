export function wsHardClipper(value){
  return Math.max(Math.min(value*4,1),-1)
}
export function wsSoftClipper(value){
  return Math.tanh(value*4) 
}
export function wsHardSync(value){
  value *= 8
  if(value > 0) return value%1 
  else if (value <0) return Math.abs(value)%1*-1 
  else return 0
}
export function wsPWM(x){
  //return x
...
export function wsSawToTri(value){
  value *= 2.3
  if(Math.abs(value) <= 1) return value
  if(value > 1) return 2-value
  if (value < -1) return -2-value 
}
export function wsTriangleFolder(value){
...
export function wsSineFolder(value){
  return Math.sin(value * Math.PI * 8);
}
export function wsPseudorandom(value){
